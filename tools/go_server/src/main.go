...
// Todo: change to standard http errors
...
func (dataPtr *DataStruct) removeFromSlice(nCells uint8, i int) {
...
func (dataPtr *DataStruct) getSlice(nCells uint8) ([]*GameSesh, error) {
	switch nCells {
...
func (dataPtr *DataStruct) appendToSlice(game *GameSesh) {
	switch (*game).NCells {
...
func (dataPtr *DataStruct) newgame(nCells string) string {
...
func (dataPtr *DataStruct) getgameinfo(rawInfo string) string {
	gameUD := new(GameSesh)
...
func (dataPtr *DataStruct) postgameinfo(rawInfo string) string {
	gameUD := new(GameSesh)
	err := json.Unmarshal(json.RawMessage(rawInfo), gameUD)
...
func (dataPtr *DataStruct) terminategame(rawInfo string) string {
	gameUD := new(GameSesh)
	err := json.Unmarshal(json.RawMessage(rawInfo), gameUD)
...
func (dataPtr *DataStruct) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "new":
		...
		answer := dataPtr.newgame(request[1])
		fmt.Fprintf(w, answer)
		break
	case "terminate":
		fmt.Fprintf(w, "terminate game request\n")
		answer := dataPtr.terminategame(request[1])
		fmt.Fprintf(w, answer)
		break
	case "get":
		fmt.Fprintf(w, "get game info request\n")
		answer := dataPtr.getgameinfo(request[1])
		fmt.Fprintf(w, answer)
		break
	case "post":
		fmt.Fprintf(w, "post game info request\n")
		answer := dataPtr.postgameinfo(request[1])
		fmt.Fprintf(w, answer)
		break
	default:
		w.WriteHeader(404)
...
func main() {
	// Todo: Refactor structure for RESTful API structure
	http.Handle("/", new(DataStruct))
